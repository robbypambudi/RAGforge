...
class VectorStoreService:
    ...
    def add_vector_store(self, path: str) -> None:
        ...
        if self.vector_store is None:
            self.vector_store = FAISS.load_local(folder_path=path, embeddings=self.embedding_model,
                                                 allow_dangerous_deserialization=True)
            logging.info(f"Vector store loaded from {path}")
            return
        else:
            vector_store = FAISS.load_local(folder_path=path, embeddings=self.embedding_model,
                                            allow_dangerous_deserialization=True)
